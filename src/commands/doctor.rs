...
pub fn run(
        ...
        match config::resolve_with_source(flag, env, from_cfg, name) {
            Ok((value, source)) => {
                checks.push(Check::ok(
                    SECTION_CONFIG,
                    name,
                    render_value(name, &value, source),
                ));
            }
            Err(_) => {
    ...
    // per-machine shared credentials exist?
    ...
    checks.push(Check::ok(
        SECTION_CONFIG,
        "LIBJFS_DIR",
        format!("{} ({libjfs_src})", truncate(&libjfs_val, 60)),
    ));
        ...
        .or_else(|| cfg.store.clone())
    {
        ...
        let detail = if report.schemas_dir.is_dir() {
            format!(
                "{} schemas in {}",
                report.schemas_present,
                truncate(&report.schemas_dir.display().to_string(), 50)
            )
        } else {
        ...
        checks.push(Check::ok(
            SECTION_CONFIG,
            "schemas",
            "n/a (no store configured)".to_string(),
        ));
    }
    ...
    checks.push(if env_present("OPENAI_API_KEY") {
        Check::ok(
            SECTION_SECRETS,
            "OPENAI_API_KEY",
            "set (needed for embed + search)",
        )
    } else {
        Check::fail(
            SECTION_SECRETS,
            "OPENAI_API_KEY",
            "not set — embed/search will fail",
        )
    });
    ...
    checks.push(if r2 {
        Check::ok(
            SECTION_SECRETS,
            "R2 credentials",
            "R2_ACCESS_KEY_ID + R2_SECRET_ACCESS_KEY set",
        )
    } else {
        Check::fail(
            SECTION_SECRETS,
            "R2 credentials",
            "R2_ACCESS_KEY_ID and/or R2_SECRET_ACCESS_KEY missing",
        )
    });
                    ...
                    Ok((pgvector, missing)) => {
                        checks.push(if pgvector {
                            ...
                            Check::fail(
                                SECTION_BACKEND,
                                "pgvector",
                                "extension `vector` not installed (run migrations)",
                            )
                        });
                        checks.push(if missing.is_empty() {
                            Check::ok(
                                SECTION_BACKEND,
                                "schema tables",
                                "blobs, file_versions, blob_chunks present",
                            )
                        } else {
                            Check::fail(
                                SECTION_BACKEND,
                                "schema tables",
                                format!("missing: {} (run migrations)", missing.join(", ")),
                            )
                        });
                    }
                }
    ...
    match (vol, met) {
        (Ok(vol), Ok(met)) => {
            ...
            match Fs::init(&vol, &met, &cache_dir) {
                Ok(_) => checks.push(Check::ok(
                    SECTION_BACKEND,
                    "storage backend",
                    format!("volume {vol:?} + object store OK"),
                )),
                Err(e) => checks.push(Check::fail(
                    SECTION_BACKEND,
                    "storage backend",
        ...
        _ => checks.push(Check::fail(
            SECTION_BACKEND,
            "storage backend",
            "volume/meta not configured — skipped (run `trove init` in the vault folder)",
        )),
    }
    ...
    match resolved_store {
        None => {
            ...
            checks.push(Check::fail(
                SECTION_VALIDATION,
                "schema lint",
                "store not configured — pass --store, set TROVE_STORE, or run `trove init` in the vault folder",
            ));
            checks.push(Check::fail(
                SECTION_VALIDATION,
                "store validation",
